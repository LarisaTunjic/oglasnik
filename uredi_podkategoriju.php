...
if ($resultCheck > 0) { 
	while ($row = mysqli_fetch_assoc($result)){
		...
		<h1 style='font-family:Satisfy;'> Uredi podkategoriju : " .$row['naziv']."</h1>";
		if(isset($_GET['error'])){
		...
		echo"<form action='dodaj_podkategoriju?id=" .$row['id_kategorija']."' method='post'>
...
if ($resultCheck > 0) { 
	...
	while ($row = mysqli_fetch_assoc($result)){
		...
		echo"<div><li><a href='uredi_podkategoriju2.php?id=" .$row['id_kategorija']."'>
			<i class='pin'></i>";
		...
		if($velicina > 15){
			...
			echo"<p class='br_ogl'>broj oglasa:" . $row['br'] ."</p>";
		if($row['slika'] == 0){
...
if(isset($_POST['urediKat'])){
	...
	if($_FILES["slika"]["error"] == 4) {
		...
		header("Location: uredi_podkategoriju.php?id=".$id."");
	}else{
		...
		if (in_array($slActualExt, $allowed)) {
			if($slError === 0){
				if($slSize < 10000000){
					...
					header("Location: uredi_podkategoriju.php?error=big_img&id=".$id);
				}
			}else{
				header("Location: uredi_podkategoriju.php?error=error_img&id=".$id);
			}
		}else{
			header("Location: uredi_podkategoriju.php?error=format&id=".$id);
		}
	}
